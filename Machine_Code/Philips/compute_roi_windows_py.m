function [ROI_positions, preSC_ROI_positions] = compute_roi_windows_py(... 
    ...
    xmap, ymap, thresholdPercentage)   % contains coordinates xmap and ymap for preSC conversion
     
    % Some axila/lateral dims
    ...
    for axial_pos = axial_start:axial_increment:axial_end
        ...
        for lateral_pos = lateral_start:lateral_increment:lateral_end
            ...
            if ( percentageOnes > thresholdPercentage )
                ...
                ROI_positions.left =[ROI_positions.left lateral(lateral_ind)]; % took out ; bettween variables (David)
                ...
                % PreSC Conversion
                preSC_ROI_positions.left = [preSC_ROI_positions.left xmap(floor(axial_ind), floor(lateral_ind))];
                preSC_ROI_positions.right = [preSC_ROI_positions.right xmap(floor(axial_ind),floor(lateral_ind+lateral_ROI_size_pixels-1))];
                preSC_ROI_positions.top = [preSC_ROI_positions.top ymap(floor(axial_ind),floor(lateral_ind))];
                preSC_ROI_positions.bottom = [preSC_ROI_positions.bottom ymap(floor(axial_ind+axial_ROI_size_pixels-1),floor(lateral_ind))];
