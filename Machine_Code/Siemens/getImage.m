function [im_array, imgData, imgInfo, refData, refInfo] = getImage(filename, filedirectory, refname, refdirectory, frame)
    ...
    %addpath(genpath('/Users/davidspector/Documents/MATLAB/'));
    addpath(genpath('/Users/elizabethszeto/Downloads/CanaryCRESTProgram/ThyroidQUS'));
    %{
    if ismac
        addpath(genpath('/Users/ahmedelkaffas/Documents/Repositories/PhilipsMatlabScripts/'))
    else
        addpath(genpath('/Users/david/Documents/MATLAB/'));
    end
    %}
    %main_parser_stanford;
    
    % CD to data
    cd('/Users/elizabethszeto/Downloads/CanaryCRESTProgram/ThyroidQUS_data/');
    %{
    if ismac
        cd('/Users/ahmedelkaffas/Documents/Repositories/PhilipsMatlabScripts/Data/');
    %     cd('/Volumes/General/FatQuantData/');
    else
        cd('/Users/david/Documents/MATLAB/Data/');
    end
    %}
...
% %     
    ...
    [imgInfo, refInfo, imgData, refData] = sie_getData(Files,refFiles, analysisParams);

    im_array = getImData(imgData.Bmode,[(imgInfo.clip_fact*imgInfo.maxval-imgInfo.dyn_range) imgInfo.clip_fact*imgInfo.maxval]);
