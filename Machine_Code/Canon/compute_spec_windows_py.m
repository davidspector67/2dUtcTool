...
%     % Output pre-allocation
      if length(top) >= 1
        im_NPS = zeros(length(top), 59);
        im_PS = zeros(length(top),59);
        im_f = zeros(length(top), 59);
        im_rPS = zeros(length(top), 59);
      ...
      for i = 1:length(top)
          ...
          imgWindow = imgRF(top(i):bottom(i),...
              left(i):right(i));
%           disp("Displaying debugging values")
%           disp(top(i))
%           disp(bottom(i))
%           disp(left(i))
%           disp(right(i))
...
%         % Method #2:
          ...
          %[f, rPS] = compute_powerSpec(refWindow, f0, f1, fs,
          %round(ref_gain)); %> (2) SUBSTRACTION: log data %***
          %NPS = PS - rPS;%> (2) SUBSTRACTION: log data %***
          NPS = PS;%> (2) SUBSTRACTION: log data %***
          ...
          %im_rPS(i,:) = rPS;%window.rPS = [window.rPS rPS];%window(i).rPS
          %= rPS; %***
